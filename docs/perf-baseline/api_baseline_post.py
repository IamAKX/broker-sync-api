...
run_dir = os.path.join(OUT_ROOT, f"{today}-post-upgrade")
...
meta = {
    ...
    "base_url": BASE,
    "note": "POST-UPGRADE. RDS db.t3.small + tuned param group; EC2 t3.small, gunicorn 3 workers; "
            "in-process response cache + LmvDailySnapshotWide read model + connection-pool "
            "hardening all deployed. Sequential (not concurrent) probe, 3 reps, pause between calls; "
            "a separate concurrent load test is in the session notes.",
