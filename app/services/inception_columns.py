...
reads from: services.inception_formula_engine (what to compute) and
routers.inception's GET /columns (what the client's field pickers show —
strategy formulas referencing these fields are evaluated entirely on the
desktop client, not here; see services/inception_strategy_store.py in the
client repo).
