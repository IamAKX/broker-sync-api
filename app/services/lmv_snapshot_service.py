...
#
# These return plain dict/list structures (already in the shape orjson will
# emit) instead of pydantic models, and run the O(days x stocks x metrics)
# pivot off the event loop via asyncio.to_thread — so a cache MISS on a
# 60-day request doesn't block every other request on that worker for the
# ~2-3s the pivot takes. The router caches the returned dict and hands it
# straight to ORJSONResponse (see app/core/cache.py, app/routers/*).
